...
if (isset($_POST["search"])) {
  // (B1) SEARCH FOR USERS
  require "pacient_diagnostic-search.php";

  echo "<table class=" . "content-table" . ">";
    echo "<thead>";
    echo "<tr>";
    echo "<th> Numar Diagnostice </th>";
  ...
  if (count($results) > 0) { foreach ($results as $r) {
   // printf("<div>%s - %s</div> - %s</div>", $r["NumeSectie"], $r["DoctorID"],  $r["NumarPersonal"]);
    printf("<tr><td>%s</td>", $r["NumarDiagnostice"]);
    printf("<td>%s</td>", $r["PacientID"]);
    printf("<td>%s</td>", $r["DiagnosticID"]);
    printf("</tr>");
  }
  printf("</tbody></table>");
} else { echo "<td>No results found</h1>"; }
}
?>